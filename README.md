...
This project is a few things, the major parts being a Pi-hole extension and a speedtest runner. Install the Speedtest Mod in a breeze with the one-liners and Docker images [below](https://github.com/town3r/pihole-speedtest?tab=readme-ov-file#installing).
...
The Test Script is a standalone tool that can be used to run speedtests on automated Debian or Fedora based systems. It will install a speedtest CLI if one hasn't been, run a test, and save the results to a new or existing database. You can use it to test your connection, monitor your ISP, or check the speed of your VPN. Let us know how you're using it! See usage for details.
    ...
    curl -sSL https://github.com/town3r/pihole-speedtest/raw/master/test | sudo bash -s -- -o ~/speedtest.db
...
The above goes for Docker as well. We use the Dockerfile in this repo to build an image with the Mod Script already applied. Simply change the image you're using to ours and proceed as usual. It's a drop-in replacement.
    ...
    ghcr.io/town3r/pihole-speedtest:latest
...
Pi-hole v6 changed dashboard rendering: widgets are loaded dynamically by `admin/scripts/pi-hole/php/content.php` from per-widget PHP partials.

This fork now vendors its installer/runner logic locally and, on v6 layouts, installs `box_speedtest.php` and patches `content.php` to include it. On older layouts, it falls back to patching `admin/index.php`. If neither layout is detected, the installer fails safely by installing only the runner and printing a warning.

Further Instructions: run `mod --reinstall` to refresh integration after Pi-hole updates, and `mod --uninstall` to remove integration while keeping historical speedtest data.
